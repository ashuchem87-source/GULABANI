...
export default function NewProjectScreen() {
  ...
  const canSave = name.trim().length > 1 && client.trim().length > 1;
  const save = async () => {
    if (!canSave) return;
    const dueDate = new Date();
    dueDate.setDate(dueDate.getDate() + Math.max(1, Number(days) || 10));
    await addProject({ name: name.trim(), client: client.trim(), summary: summary.trim() || 'A new project ready to move through your system.', templateId, dueDate: dueDate.toISOString(), reminderFrequency: frequency });
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: Platform.OS === 'web' ? 24 : insets.top + 14, paddingBottom: insets.bottom + 34 }]} keyboardShouldPersistTaps="handled">
        ...
        <View style={[styles.preview, { backgroundColor: colors.secondary }]}><Feather name="zap" size={16} color={colors.primary} /><View style={{ flex: 1 }}><AppText style={styles.previewTitle}>{selectedTemplate.name} is ready</AppText><AppText style={[styles.previewText, { color: colors.mutedForeground }]}>{selectedTemplate.steps.length} steps will be assigned automatically, starting with “{selectedTemplate.steps[0].title}”.</AppText></View></View>
